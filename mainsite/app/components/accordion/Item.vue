<template>
  <div ref="accordionItemEl" class="relative group w-auto h-auto p-10 border-2 border-red-500 rounded-3xl space-y-5 cursor-pointer" @click="select(index)">
    <div class="rounded-full h-10 w-10 bg-red-500 text-red-50 font-bold flex items-center justify-center absolute -top-5 left-10 group-hover:scale-105 group-hover:shadow-md tranition-scale duration-500">
    ...
    <div class="font-bold flex justify-between items-center">
      ...
      <div class="rounded-full w-10 h-10 flex items-center justify-center font-bold bg-red-500 text-red-50">
        {{ index }}
      </div>
    </div>
